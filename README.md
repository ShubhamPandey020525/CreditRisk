...
We selected **XGBoost** (`XGBClassifier`) over traditional Logistic Regression or Random Forests for its superior handling of non-linear financial interactions (e.g., detecting the specific risk of high age combined with a massive medical loan). 
- **Objective:** `binary:logistic` (outputs the log-odds of a default).
